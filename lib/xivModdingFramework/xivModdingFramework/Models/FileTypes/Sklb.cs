...
namespace xivModdingFramework.Models.FileTypes
{
    ...
    public static class Sklb
    {
        ...
        private static async Task<string> ConvertSkelToXml(string rawFile)
        {
            ...
            var applicationName = "NotAssetCc.exe";
            var extraFlags = "";
            ...
            if(ContainsUnicodeCharacter(cwd))
            {
                ...
                if(File.Exists(Path.Combine(cwd, "AssetCc2.exe")))
                {
                    applicationName = "AssetCc2.exe";
                    extraFlags = "-s ";
                ...
                } else
                {
                    ...
                    {
                        // Temp folder ALSO has unicode — try a short ASCII path under /tmp or C:\.
                        var rootDir = PlatformPaths.IsWindows
                            ? new DriveInfo("C").RootDirectory.FullName
                            : Path.DirectorySeparatorChar.ToString();
                        var guid = Guid.NewGuid().ToString();
                        tempFileXml = Path.Combine(rootDir, guid + ".xml");
                        tempFileRaw = Path.Combine(rootDir, guid + ".sklb");
            ...
            var applicationPath = NativeConverters.ResolveExecutable(cwd, applicationName, Path.GetFileNameWithoutExtension(applicationName));
            var proc = new Process
            {
                StartInfo = NativeConverters.CreateStartInfo(
                    applicationPath,
                    extraFlags + "\"" + rawFile + "\" \"" + xmlFile + "\"",
                    cwd)
            };
            proc.Start();
            proc.WaitForExit();
