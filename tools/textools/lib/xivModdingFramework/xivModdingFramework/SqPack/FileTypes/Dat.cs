...
namespace xivModdingFramework.SqPack.FileTypes
{
    ...
    public static class Dat
    {
        ...
        public static long GetMaximumDatSize()
        {
            ...
            {
                // Check the user's FFXIV installation volume to see what the maximum file size is for their file system.
                // Use Path.GetPathRoot so Linux paths like /home/... work (Substring(0,1) only works for Windows drive letters).
                var drive = Helpers.PlatformPaths.GetPathRoot(XivCache.GameInfo.GameDirectory.FullName);
                return GetMaximumFileSize(drive);
            }
        }

        // Returns the maximum file size in bytes on the filesystem type of the specified drive/volume root.
        private static long GetMaximumFileSize(string drive)
        {
            try
            {
                ...
                switch (driveInfo.DriveFormat)
                {
                    ...
                    case "FAT32":
                        return 4294967296;
                    default:
                        // Unknown / Linux filesystem, default to the max index limit.
            ...
            {
                // DriveInfo can throw on unusual mounts; index limit is a safe ceiling.
                return 34359738368;
            }
        }
