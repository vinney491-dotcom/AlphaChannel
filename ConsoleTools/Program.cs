...
namespace ConsoleTools
{
    public class ConsoleTools
    {
        ...
        {
            // Legacy Windows layout kept optional deps under ./lib — still honor that if present.
            var entryDir = Path.GetDirectoryName(Assembly.GetEntryAssembly()!.Location) ?? ".";
            var cwd = Path.Combine(entryDir, "lib");
            ...
            // Native convenience: allow XIV_PATH / --xiv-path without editing console_config.json
            args = ApplyPathOverrides(args);
        ...
        /// <summary>
        /// Applies XIV path overrides and returns args with global flags removed.
        /// </summary>
        private static string[] ApplyPathOverrides(string[] args)
        {
            string? path = null;
            var kept = new System.Collections.Generic.List<string>();
            for (int i = 0; i < args.Length; i++)
            {
                if ((args[i] == "--xiv-path" || args[i] == "/xivpath") && i + 1 < args.Length)
                {
                    path = args[i + 1];
                    i++; // skip value
                    continue;
            ...
            path ??= Environment.GetEnvironmentVariable("XIV_PATH")
                     ?? Environment.GetEnvironmentVariable("FFXIV_PATH");
        ...
        public static async Task<int> Run(string[] args)
        {
            ...
            // Help / doctor do not require a game install.
            var cmd = args.Length > 0 ? args[0] : "/?";
            if (cmd == "/?" || cmd == "--help" || cmd == "-h" || cmd == "/doctor" || cmd == "--doctor")
            {
        ...
        private static int ShowDoctor()
        {
            Console.WriteLine("==== AlphaChannel TexTools — native doctor ====");
            Console.WriteLine($"OS: {(PlatformPaths.IsLinux ? "Linux" : PlatformPaths.IsWindows ? "Windows" : PlatformPaths.IsMacOS ? "macOS" : "unknown")}");
            Console.WriteLine($"Runtime: {System.Runtime.InteropServices.RuntimeInformation.FrameworkDescription}");
            Console.WriteLine($"Launcher config root: {PlatformPaths.GetLauncherConfigRoot()}");
            Console.WriteLine($"launcher.ini: {(File.Exists(PlatformPaths.GetLauncherIniPath()) ? PlatformPaths.GetLauncherIniPath() : "missing")}");
            Console.WriteLine($"launcherConfigV3.json: {(File.Exists(PlatformPaths.GetLauncherConfigV3Path()) ? "found" : "missing (normal on Linux XLCore)")}");
            Console.WriteLine($"Resolved game path: {ConsoleConfig.ResolveDefaultXivPath() ?? "(none)"}");
            Console.WriteLine($"TexTools data root: {PlatformPaths.GetTexToolsDataRoot()}");
            Console.WriteLine($"Configured XivPath: {ConsoleConfig.Get().XivPath}");
            Console.WriteLine($"Env XIV_PATH: {Environment.GetEnvironmentVariable("XIV_PATH") ?? "(unset)"}");
            Console.WriteLine("");
            Console.WriteLine("Probed game roots:");
            foreach (var root in PlatformPaths.EnumerateGameRootCandidates())
            {
                var mark = Directory.Exists(root) ? "OK " : " --";
                Console.WriteLine($"  {mark} {root}");
            }
            if (ConsoleConfig.ResolveDefaultXivPath() == null)
            {
                Console.WriteLine("");
                Console.WriteLine("No game found. On CachyOS/XLCore, either install/launch the game once, or set:");
                Console.WriteLine("  export XIV_PATH=\"$HOME/.xlcore/ffxiv/game/sqpack/ffxiv\"");
                Console.WriteLine("Find installs with:");
                Console.WriteLine("  find ~ -type d -path '*/game/sqpack/ffxiv' 2>/dev/null");
            }
            return 0;
        }
